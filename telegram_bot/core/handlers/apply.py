...
async def start_submit_apply(message: Message, applications_middleware: ApplicationsMiddleware, user_language,
    ...
    if is_user is None:
        ...
        await message.answer(translate_text("Do you give your consent to the processing of your personal data, as well "
        ...
        await message.answer(translate_text("You cannot resubmit your application while your previous application"
...
async def finish_consent(message: Message, state: FSMContext):
    ...
    await message.answer(translate_text("You cannot submit an application without consent to the processing of your"
...
async def process_consent(message: Message, state: FSMContext):
    ...
    await message.answer(translate_text("Thank you for your agreement! Now let's gather the necessary information.",
                                        'en', user_language))
    
    await message.answer(translate_text(f"Please note:\n- Entry into the country: after 01.02.{current_year}!\n"
                                        f"- The training lasts until the end of June {current_year+1} years!",
                                        'en', user_language))
    
    await message.answer(translate_text("Enter your last name.", 'en', user_language))
...
async def process_personal_info(message: Message, state: FSMContext):
    ...
    if 'last_name' not in data:
        if message.text.isalpha():
            ...
            await message.answer(translate_text("Great! Now enter your name.", 'en', user_language))
        else:
            await message.answer(translate_text("The last name must contain only letters!", 'en', user_language))

    elif 'first_name' not in data:
        if message.text.isalpha():
            await state.update_data(first_name=message.text.capitalize())
            await message.answer(translate_text("Enter the country in which you live.", 'en', user_language))
        else:
            await message.answer(translate_text("The name must contain only letters!", 'en', user_language))

    elif 'country' not in data:
        if message.text.isalpha():
            await state.update_data(country=message.text.capitalize())
            await message.answer(translate_text("Enter your date of birth (in MM-DD-YYYY format).", 'en',
                                                user_language))
        else:
            await message.answer(translate_text("The name of the country must contain only letters!", 'en',
                                                user_language))

    elif 'date_of_birth' not in data:
        if not date_pattern.match(message.text):
            await message.answer(
                translate_text("Please enter your date of birth in MM-DD-YYYY format.", 'en', user_language))
        else:
            ...
            if 16 <= age < 18:
                ...
                await message.answer(translate_text("**Please note that if you are under 18 years old, then you must"
                ...
                await state.update_data(date_of_birth=message.text)
                await message.answer(translate_text("Enter your contact phone number.", 'en', user_language))
                
            else:
                await message.answer(translate_text("Age limits for submitting an application: 18+ years old!", 'en',
        ...
        if await validate_phone_number(message.text):
            ...
            await message.answer(translate_text("Enter your email address.", 'en', user_language))
        else:
            await message.answer(translate_text("An incorrect phone number has been entered!", 'en', user_language))

    elif 'email' not in data:
        if await is_valid_email(message.text):
            await state.update_data(email=message.text)
            await message.answer(translate_text("Which country did you study in before?", 'en', user_language))
        else:
            await message.answer(translate_text("You have entered an incorrect email!", 'en', user_language))

    elif 'previous_education_country' not in data:
        if message.text.isalpha():
            await state.update_data(previous_education_country=message.text.capitalize())
            await message.answer(translate_text("Send a passport photo in PDF or JPG format.", 'en', user_language))
...
async def process_passport_data(message: Message, bot: Bot, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(translate_text("Send your passport in JPG or PDF format!", 'en', user_language))
        return
    ...
    await message.answer(translate_text("Send a photo of your passport translated into Russian in PDF or JPG format.",
                                        'en', user_language))
...
async def process_ru_passport_data(message: Message, bot: Bot, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(translate_text("Send your passport translated into Russian in JPG or PDF format!",
                                            'en', user_language))
        return
    ...
    await message.answer(translate_text("Send a photo of the completed and signed visa application form in PDF or"
...
async def process_visa_application_form_data(message: Message, bot: Bot, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(translate_text("Send the completed and signed visa application form in PDF or JPG format!",
                                            'en', user_language))
        return
    ...
    await message.answer(translate_text("Send a photo of a bank statement in your name with an account balance of"
...
async def process_bank_statement_data(message: Message, bot: Bot, state: FSMContext):
    ...
    if message.photo:
        ...
        await message.answer(translate_text("Send a bank statement in your name with an account balance of $4,500 or"
    ...
    await message.answer(translate_text("Are you applying for yourself?", 'en', user_language),
...
async def process_application_type(message: Message, state: FSMContext):
    ...
    await message.answer(translate_text("Would you like to add any comments to your application? If yes, write it in "
...
async def process_agree_comments(message: Message, state: FSMContext):
    ...
    await message.answer(translate_text("Do you really want to apply?", 'en', user_language),
...
async def finish_apply(message: Message, applications_middleware: ApplicationsMiddleware, bot: Bot, user_language,
    ...
    if 'Yes' in message.text:
        ...
        if result:
            await message.answer(translate_text("Thanks! Your application has been submitted for consideration.",
                                                'en', user_language),
                                 reply_markup=menu_keyboard)
            ...
            await message.answer(translate_text("An error in submitting the application! Try again, or contact support",
                                                'en', user_language), reply_markup=contact_with_operator_keyboard)

    else:
        await message.answer(translate_text("Your application has not been saved!", 'en', user_language),
                             reply_markup=menu_keyboard)
    await state.clear()
